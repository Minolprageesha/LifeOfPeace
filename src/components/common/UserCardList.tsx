...
const DocterListSection: React.FC = () => {
  ...
  const [selectedDoctor, setSelectedDoctor] = useState<UserCard | undefined>(undefined);
  ...
  return (
    <div style={{ flex: 0.73, marginTop: "2rem", display: "flex", justifyContent: "flex-start", gap: "1rem", flexDirection: "column", padding: "1rem" }}>
      ...
      <div style={{ marginTop: "2rem", display: "flex", justifyContent: "flex-start", gap: "1rem", flexWrap: "wrap", padding: "1rem" }}>
        {/* Trending Movies */}
        {!isDetailPage && option === Options.subcribe && <SubcribedUsers selectedDocterHandler={selectedDoctorHandler} doctors={subcribedDoctors} />}

        {/* Upcoming Movies */}
        {!isDetailPage && option === Options.unsubcribe && <AllDoctors selectedDocterHandler={selectedDoctorHandler} doctors={allDoctors} />}
...
const TabBar = (props) => {
  ...
  return (
    <div className="navbar-other flex ms-lg-4 aliganItem" style={{ marginTop: "5rem" }}>
      <h1
        className={all ? "btn-main" : "btn-sub"}
        onClick={() => {
          setAll(true);
          setOption(Options.unsubcribe);
          setIsDetailPage(false);
